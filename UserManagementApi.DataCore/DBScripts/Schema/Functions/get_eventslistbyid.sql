CREATE OR REPLACE FUNCTION public.get_eventslistbyid(
	_eventid integer)
    RETURNS TABLE(event_id integer, name character varying, descr character varying, eventdate timestamp with time zone, bump_filing_due boolean, filername character varying, filer_type_id character varying, event_filer_map_id integer) 
    LANGUAGE 'plpgsql'
    COST 100
