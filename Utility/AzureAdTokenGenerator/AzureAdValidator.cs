...
namespace Denver.Infra
{
    public class AzureAdValidator
    {
        ...
        public AzureAdValidator(IPublicClientApplication app)
        {
            App = app;
        }

        /// <summary>
        /// Acquires a token from the  Username/password
        /// </summary>
        /// <returns>An AuthenticationResult if the user successfully signed-in, or otherwise <c>null</c></returns>
        public async Task<ADAuthenticationResult> AcquireAzureADTokenFromUsernamePasswordAsync(IEnumerable<String> scopes, string username, SecureString password)
        {
            ...
            if (result == null)
            {
                return await GetTokenForWebApiUsingUsernamePasswordAsync(scopes, username, password);
            }

            return new ADAuthenticationResult { Result = result };
        }
        ...
        private async Task<ADAuthenticationResult> GetTokenForWebApiUsingUsernamePasswordAsync(IEnumerable<string> scopes, string username, SecureString password)
        {
            ADAuthenticationResult adResult = new ADAuthenticationResult();            
            try
            {
                adResult.Result = await App.AcquireTokenByUsernamePassword(scopes, username, password)
                    .ExecuteAsync();
            }
            catch (MsalUiRequiredException ex)
            {
                adResult.Message = ex.Message;
            ...
            {
                adResult.Message = "U/P: Wrong username";
            ...
            {
                adResult.Message = "trust_response_failed";
                // ------------------------------------------------------------------------
